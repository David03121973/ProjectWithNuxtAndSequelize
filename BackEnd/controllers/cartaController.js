...
const updateCarta = async (req, res) => {
  const { id } = req.params;
  const { nombre, tipo, descripcion, raza, arquetipo, imagen } = req.body;

  // Validar que se proporcione al menos un campo para actualizar
  if (!nombre && !tipo && !descripcion && !raza && !arquetipo && !imagen) {
  ...
  try {
      ...
      if (nombre) {
          updatedCartaData.nombre = nombre;
      }

      // Actualizar el tipo si se proporciona
      if (tipo) {
          updatedCartaData.tipo = tipo;
      }

      // Actualizar la descripción si se proporciona
      if (descripcion) {
          updatedCartaData.descripcion = descripcion;
      }

      // Actualizar la raza si se proporciona
      if (raza) {
          updatedCartaData.raza = raza;
      }

      // Actualizar el arquetipo si se proporciona
      if (arquetipo) {
          updatedCartaData.arquetipo = arquetipo;
      }

      // Actualizar la imagen si se proporciona
      if (imagen) {
          updatedCartaData.imagen = imagen;
      }
